      ...
      <div className="relative max-w-[1400px] mx-auto px-8 py-8">
        ...
        <div className="mb-6 border border-amber-400/40 bg-amber-500/5 p-4 flex items-start gap-3">
          ...
          <div className="flex-1 text-sm text-stone-200 leading-relaxed">
            ...
            评分代表"产业链位置"，不代表"可以买"。52W 标尺已全面升级为高精指针模式，超过 85% 水位将亮红预警。
          </div>
        </div>
...
function StockCard({ stock, expanded, onToggle }) {
  ...
  // 用于计算高位指针的物理百分比位置
  const rangePct = (s.week52High && s.week52Low && s.week52High !== s.week52Low)
  ...
  return (
    <div className={`border ${cardStyle} p-4 hover-lift cursor-pointer`} onClick={onToggle}>
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-baseline gap-3 mb-1">
            ...
            {isHighZone && (
              <span className="px-1 py-0.5 text-[9px] bg-rose-500/20 text-rose-400 border border-rose-500/40 rounded animate-pulse font-bold font-mono">
                高位
              </span>
            )}
          </div>
      ...
      {/* 52W 物理指针样式组件注入 */}
      <PriceSnapshot stock={s} rangePct={rangePct} isHighZone={isHighZone} />
...
function PriceSnapshot({ stock, rangePct, isHighZone }) {
  const s = stock;
      ...
      <div className="mb-1">
        <div className="flex items-center gap-2 text-[10px] text-stone-400">
          <div className="flex-1 h-1.5 bg-stone-800 relative rounded-full overflow-hidden">
            ...
            <div 
              className={`absolute h-full w-1 z-10 transition-all duration-500 ${
                isHighZone ? 'bg-rose-400 shadow-[0_0_8px_#fb7185]' : 'bg-white shadow-[0_0_6px_#fff]'
              }`}
              style={{ left: `${Math.min(100, Math.max(0, rangePct))}%` }} 
            />
          </div>
          <span className="font-mono text-[9px] uppercase min-w-[75px] text-right">
            52W刻度: <span className={isHighZone ? "text-rose-400 font-bold" : "text-amber-200"}>{rangePct.toFixed(0)}%</span>
          </span>
        </div>
      </div>
      {zoneWarning && (
  ...
  return (
    <div className={`border ${stageStyle} p-4 hover-lift`}>
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="text-right">
          <div className="flex items-baseline gap-1 justify-end">
            <span className="font-display digit text-2xl text-stone-50">{c.score}<span className="text-stone-500 text-base">/10</span></span>
          </div>
